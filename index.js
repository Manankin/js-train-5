...
function printUppercase(array) {
  ...
  // another way of solution without changing array
  // for (const item of array) {
  //   console.log(item.toUpperCase())
  // }
}
...
function findMinMax([firstElement, ...restElements]) {
  ...
  for (let i = 0; i < restElements.length; i++) {
    minItem = (minItem > restElements[i]) ? restElements[i] : minItem;
    maxItem = (maxItem < restElements[i]) ? restElements[i] : maxItem;
  }
...
findMinMax([5, 1, 8, 2, 9, 3]); // Виведе Min: 1 Max: 9
...
function addTenToEach(array) {
  ...
  let newArray = Array(array.legth);
...
function replaceNumbers(arr) {
  ...
  for (const index in arr) {
    (arr[index] <= 10) ? replaceArray[index] = "Less than or equal to 10" : replaceArray[index] = "Greater than 10";
  }
  // Створюємо пустий масив для зберігання результату
  // Проходимо через кожен елемент вхідного масиву
